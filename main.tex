...
\title{온라인 게임 동시접속자 수요 구조의 함수형 분석: Path of Exile의 리그 효과와 이상치 탐지}
...
\begin{abstract}
본 연구는 Steam 공개 동시접속자 데이터를 이용하여 \textit{Path of Exile}의 일중(intraday) 동접 곡선을 함수형 데이터로 분석하였다. 각 날짜의 24시간 동접 시계열을 하나의 함수로 보고, B-spline 평활화, 함수형 주성분 분석(FPCA), 함수형 회귀, 1차 미분 함수 기반 보조 분석, 함수형 이상치 탐지를 적용하였다. 분석 결과, 일중 곡선의 변동은 사실상 하나의 모드가 주도하는 단순한 구조로 나타났다. 첫 번째 함수형 주성분이 전체 변동의 94.47\%를 설명하였으며, 이는 곡선의 모양을 유지한 채 전체 수준만 위아래로 이동시키는 수준 효과(level effect)로 해석되었다. 두 번째 함수형 주성분은 설명 비율 자체는 작지만, 리그 출시일에 집중적으로 나타나는 단기 이벤트 효과를 포착하는 보조 모드 역할을 하였다. 또한 주말, 리그 출시일, 리그 출시 후 첫 주 및 첫 달의 효과를 시간대별 함수로 추정한 결과, 리그 출시일은 출시 전 점검 시간의 음의 효과와 출시 후 저녁 시간대의 폭발적 양의 효과를 동시에 갖는 매우 비대칭적인 패턴을 보였다. 추가적인 1차 미분 함수 분석에서는 원래 동접 곡선과 달리 변동이 여러 모드로 분산되었고, 출시일의 점검 이후 급격한 회복 구조가 변화율 관점에서 더 직접적으로 나타났다. Outliergram과 MS plot을 결합한 함수형 이상치 탐지에서는 강력한 이상치 93개를 확인하였고, 분석 가능한 모든 리그 출시일이 이상치로 검출되었다. 본 연구는 함수형 데이터 분석이 게임 서비스의 수요 구조, 이벤트 효과, 비정상 날짜 패턴을 분해하고 해석하는 데 유용함을 보여준다.
\end{abstract}

\noindent\textbf{주요어:} 함수형 데이터 분석, 온라인 게임, 동시접속자 데이터, 함수형 주성분 분석, 함수형 회귀, 함수형 미분, 함수형 이상치 탐지
...
본 연구는 접속자 수의 증감 여부에서 한 걸음 더 나아가, 변화 속도와 증감 전환 시점을 포착하기 위해 미분 함수 분석을 함께 도입한다. 특정 시간대의 수요 수준을 나타내는 $x(t)$와 달리, 1차 미분 함수 $x'(t)$는 동접의 증감 속도를 보여준다. 본 연구는 동접 곡선 자체를 주된 분석 대상으로 삼되, 1차 미분 함수를 보조적으로 활용하여 변화의 동학을 함께 해석한다.

\subsection{연구 대상}
본 연구는 \textit{Path of Exile}(이하 PoE)의 Steam 공개 동시접속자 데이터를 사용한다. PoE는 약 3개월 주기의 리그(league) 출시로 잘 알려져 있으며, 새 리그 출시는 동접의 급격한 증가를, 리그 후반으로 갈수록 점진적인 감소를 동반한다. 이처럼 뚜렷한 이벤트 구조는 FPCA, 함수형 회귀, 이상치 탐지와 같은 함수형 분석 기법을 적용하기에 적합한 환경을 제공한다.

\subsection{연구 질문}
본 연구는 다음 세 가지 주 분석 질문과 하나의 보조 분석 질문을 다룬다.
\begin{enumerate}[label=(\arabic*)]
    ...
    \item 보조적으로, 동접 곡선의 1차 미분 함수는 원래 동접 곡선 분석과 어떤 보완적 정보를 제공하는가?
\end{enumerate}
...
\subsection{미분 함수 기반 보조 분석}
동접이 언제 증가하거나 감소하는지에 초점을 맞추기 위해, 평활화된 함수 $x_i(t)$의 1차 미분 함수 $D x_i(t)$도 보조적으로 분석하였다. 본 연구의 반응변수는 $\log(1+\mathrm{players})$이므로, $D x_i(t)$는 시간대별 로그 동접의 변화율, 즉 상대적 증가/감소 속도로 해석할 수 있다. 양수 값은 해당 시간대에 로그 동접이 증가하고 있음을, 음수 값은 감소하고 있음을 의미한다. 미분 함수는 원자료를 직접 차분하지 않고 B-spline으로 평활화된 함수에 대해 계산하였다. 이는 5분 단위 관측의 국소적 변동이 미분 과정에서 증폭되는 것을 완화하기 위해서이다.
...
\subsection{함수형 회귀}
주말과 리그 상태가 날짜별 곡선에 미치는 영향을 보기 위해 함수형 회귀(function-on-scalar regression)을 적용하였다. 리그 관련 변수는 세 개의 리그 상태를 모두 별도로 더미화한 것이 아니라, 네 범주를 갖는 하나의 범주형 변수로 정의하였다. 구체적으로 리그 상태 $G_i$는
\[
G_i \in \{\mathrm{Regular/Late},\; \mathrm{LeagueDay},\; \mathrm{LeagueWeek1},\; \mathrm{LeagueMonth1}\}
\]
의 네 범주를 가지며, \textit{Regular/Late}를 기준범주(reference category)로 두었다. 따라서 회귀모형에 포함되는 LeagueDay, LeagueWeek1, LeagueMonth1은 기준범주 대비 효과를 나타내는 더미 변수이다.

\begin{equation}
\begin{aligned}
    y_i(t) =\;& \beta_0(t)
    + \beta_W(t)\,\mathrm{Weekend}_i
    + \gamma_D(t)\,I(G_i=\mathrm{LeagueDay}) \\
    &+ \gamma_{W1}(t)\,I(G_i=\mathrm{LeagueWeek1})
    + \gamma_{M1}(t)\,I(G_i=\mathrm{LeagueMonth1})
    + \epsilon_i(t).
\end{aligned}
\end{equation}
여기서 기준범주인 \textit{Regular/Late}는 리그 출시 후 30일 이후의 일반적인 운영 기간을 의미하며, 세 리그 관련 더미 변수가 모두 0인 경우에 해당한다. 따라서 절편 함수 $\beta_0(t)$는 평일이면서 Regular/Late 기간인 기준 곡선을 나타내고, $\gamma_D(t)$, $\gamma_{W1}(t)$, $\gamma_{M1}(t)$는 각각 이 기준범주 대비 출시일, 출시 후 첫 주, 출시 후 첫 달의 추가 효과를 나타낸다. 이와 같이 한 범주를 기준범주로 제외했기 때문에 식별성 문제는 발생하지 않는다.

각 회귀계수 함수는 B-spline basis로 확장하고 roughness penalty를 적용하여 추정하였다. 추정에는 \texttt{fda::fRegress()}를 사용하였다.

\begin{table}[H]
\centering
\caption{함수형 회귀에 사용한 설명 변수와 기준범주}
\begin{tabular}{lll}
\toprule
변수 또는 범주 & 정의 & 모형상 역할 \\
\midrule
Weekend & 토요일 또는 일요일이면 1, 그 외 0 & 달력 효과 \\
Regular/Late & 리그 출시 후 30일 이후 기간 & 리그 상태 기준범주 \\
LeagueDay & 리그 출시일 당일 & Regular/Late 대비 효과 \\
LeagueWeek1 & 리그 출시 후 1--6일 & Regular/Late 대비 효과 \\
LeagueMonth1 & 리그 출시 후 7--29일 & Regular/Late 대비 효과 \\
\bottomrule
\end{tabular}
\end{table}
...
표 \ref{tab:fpca_var}에 따르면 첫 번째 주성분은 전체 변동의 94.47\%를 설명하고, 두 번째 주성분까지 누적 97.58\%를 설명한다. 이는 PoE 일중 곡선의 변동이 사실상 하나의 수준 모드로 요약되며, 두 번째 주성분은 설명 비율은 작지만 리그 출시일 이벤트를 식별하는 보조 모드로 기능함을 의미한다.
...
그림 \ref{fig:fpca_pm}에서 FPC1은 평균 곡선을 거의 평행하게 위아래로 이동시키는 수준 효과로 해석된다. 반면 FPC2는 저녁 피크의 강도와 위치를 조절하는 모드로, 리그 출시일과 같은 특수 날짜에서 크게 나타난다.
...
\caption{원래 곡선 $\log(1+\mathrm{players})$에 대한 함수형 회귀 효과 함수와 95\% 신뢰구간. 이 그림은 각 설명변수가 시간대별 동접 수준에 미치는 영향을 나타낸다.}
\label{fig:fosr_effects}
\end{figure}

그림 \ref{fig:fosr_effects}의 리그 관련 효과는 모두 Regular/Late 기준범주 대비 효과로 해석한다. 주말 효과는 하루 전반에 걸쳐 양수이지만 특히 UTC 09--12시에서 크게 나타난다. 리그 출시일 효과는 가장 극적이다. UTC 17시 이전에는 기준범주보다 낮은 음수 효과(점검 시간), UTC 19시 이후에는 강한 양수 효과(출시 후 폭증)를 보이며, 하루 전체 평균으로는 설명되지 않는 비대칭 구조를 드러낸다. 리그 첫 주와 첫 달의 효과는 비교적 평행 이동형이며, 첫 주의 효과가 첫 달보다 크다.

\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\linewidth]{figures/fig07_timevarying_r2.pdf}
\caption{시간대별 결정계수 $R^2$. 주말 및 리그 상태 변수만으로도 일중 곡선 변동의 상당 부분을 설명한다.}
\label{fig:r2}
\end{figure}

\begin{table}[H]
\centering
\caption{시간대 구간별 결정계수 $R^2(t)$ 요약}
\begin{tabular}{lccc}
\toprule
시간대(UTC) & 평균 $R^2$(\%) & 최소 $R^2$(\%) & 최대 $R^2$(\%) \\
\midrule
00--06 & 58.1 & 56.3 & 59.9 \\
06--12 & 61.0 & 59.2 & 62.5 \\
12--18 & 63.0 & 60.7 & 64.4 \\
18--24 & 57.8 & 50.9 & 63.2 \\
\bottomrule
\end{tabular}
\end{table}

시간대별 결정계수의 평균은 60.2\%이며, 최대는 UTC 17.42시 부근, 최소는 UTC 22.50시 부근에서 나타났다. 주말과 리그 관련 변수만으로도 PoE 동접 곡선의 주요 변동을 상당 부분 설명할 수 있음을 보여준다. 구간별로는 12--18시의 평균 설명력이 63.0\%로 가장 높았고, 18--24시는 UTC 22시 부근의 급격한 변동으로 인해 평균 설명력이 57.8\%로 낮아졌다.


\subsection{미분 함수 기반 보조 분석}

동접 곡선 자체는 특정 시간대의 수요 수준을 나타내는 반면, 미분 함수는 접속자가 증가하거나 감소하는 속도를 나타낸다. 따라서 동접이 언제 증가하거나 감소하는지에 대한 질문에는 미분 함수가 더 직접적인 정보를 제공한다.


\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.82\linewidth]{figures/fig12_derivative_mean.pdf}
\caption{평활화된 일중 동접 곡선의 평균 미분 함수와 $\pm 1$ 표준편차. 양수 값은 해당 시간대에 로그 동접이 증가함을, 음수 값은 감소함을 의미한다. 음영은 신뢰구간이 아니라 날짜별 미분 함수의 변동성을 나타낸다.}
\label{fig:derivative_mean}
\end{figure}

그림 \ref{fig:derivative_mean}에서 평균 미분 함수는 UTC 0--6시 구간에서 음수, UTC 7--19시 구간에서 양수, UTC 20시 이후에서 다시 음수로 나타난다. 이는 평균 동접 곡선이 UTC 06--07시에 저점을 형성한 뒤 UTC 19--20시까지 증가하고, 이후 감소하는 구조와 일치한다. 특히 UTC 20시 이후에는 표준편차가 커지는데, 이는 저녁 피크 이후 시간대와 점검 시간대에서 날짜별 증가 및 감소 패턴이 크게 달라짐을 의미한다.

\begin{table}[H]
\centering
\caption{미분 함수의 함수형 주성분 설명 비율}
\begin{tabular}{ccc}
\toprule
FPC & 설명 비율 & 누적 설명 비율 \\
\midrule
...
\bottomrule
\end{tabular}
\label{tab:derivative_fpca}
\end{table}

표 \ref{tab:derivative_fpca}는 미분 함수에 대한 FPCA 결과를 요약한다. 원래 동접 곡선에서는 첫 번째 주성분이 94.47\%를 설명한 반면, 미분 함수에서는 첫 번째 주성분의 설명 비율이 22.67\%에 그치고 처음 네 개 주성분의 누적 설명 비율도 62.15\%이다. 이는 미분 과정에서 전체 수준 차이가 상당 부분 제거되고, 증가 및 감소 속도와 시점 차이가 여러 모드로 분산되기 때문이다. 따라서 원래 곡선은 수요 수준과 리그 사이클을 설명하는 데 적합하고, 미분 함수는 변화율 구조를 설명하는 데 더 적합하다.

\begin{figure}[H]
\centering
\maybeincludegraphics[width=0.98\linewidth]{figures/fig13_derivative_effects.pdf}
\caption{미분 함수에 대한 함수형 회귀 효과. 원래 곡선의 회귀계수가 특정 시간대의 수준 차이를 나타낸다면, 미분 함수의 회귀계수는 특정 시간대의 증가 및 감소 속도 차이를 나타낸다. 미분 함수의 점별 신뢰구간은 평활화 모델로부터 별도로 유도하거나 부트스트랩으로 추정해야 하는데, 본 분석은 보조적 성격이므로 점추정만 제시하였다.}
\label{fig:derivative_effects}
\end{figure}

그림 \ref{fig:derivative_effects}에서 League day 효과는 UTC 16--18시 부근에서 음수, UTC 19--20시 부근에서 큰 양수로 나타난다. 이는 리그 출시일에 점검 구간 직후 동접이 급격히 회복 및 상승하는 전환 구간을 미분 함수가 직접 포착한 결과이다. 반면 League week 1과 League month 1 효과는 미분 함수에서는 상대적으로 작고 여러 시간대에서 진동하는 형태로 나타난다. 이는 해당 효과들이 변화율 효과라기보다 원래 동접 곡선 전체를 위로 이동시키는 수준 효과에 가깝다는 기존 해석을 뒷받침한다.
...
Other & 54 & 정의된 유형에 명확히 매칭되지 않는 이상치 \\
\bottomrule
\end{tabular}
...
\section{논의}
% =========================================================
본 연구는 PoE의 일중 동시접속자 곡선에 함수형 데이터 분석을 적용하여 네 가지 핵심 결과를 얻었다. 첫째, PoE의 일중 변동은 매우 단순하다. 첫 번째 함수형 주성분 하나가 전체 변동의 94.47\%를 설명하며, 이는 곡선 모양의 변화보다 전체 수준의 변동이 훨씬 크다는 뜻이다. 둘째, 함수형 분석은 시간 척도가 다른 효과를 자연스럽게 분해한다. FPCA에서는 90일에 걸친 리그 사이클 효과와 단 하루의 출시일 효과가 서로 다른 주성분으로 분리되었고, 함수형 회귀에서는 주말 효과, 출시일 효과, 첫 주 및 첫 달 효과가 시간대별 함수로 명시적으로 추정되었다. 셋째, 미분 함수 기반 보조 분석은 기존 결과와 상충하기보다 보완적인 정보를 제공하였다. 원래 동접 곡선 분석은 리그 사이클과 첫 주와 첫 달의 지속적 수준 상승을 잘 포착한 반면, 미분 함수 분석은 리그 출시일의 점검 이후 급격한 회복과 같은 변화율 구조를 더 직접적으로 보여주었다. 넷째, 함수형 이상치 탐지는 게임 운영에서 중요한 날짜를 자동으로 식별한다. 리그 출시일은 모두 이상치로 검출되었고, 점검일과 피로 구간도 다른 유형으로 구분되었다.
...
\section{결론}
% =========================================================
본 연구는 PoE의 일중 동시접속자 곡선을 함수형 데이터로 보고, 평활화, FPCA, 함수형 회귀, 미분 함수 기반 보조 분석, 함수형 이상치 탐지를 적용하였다. 그 결과 PoE의 일중 변동은 주로 수준 변화로 설명되며, 리그 사이클과 출시 이벤트가 서로 다른 시간 척도의 변동으로 분리되고, 이상치 탐지는 출시일과 점검일을 효과적으로 식별함을 확인하였다. 또한 미분 함수 분석은 원래 동접 곡선이 포착한 수준 효과와 달리, 출시일 점검 이후 급격한 회복과 같은 변화율 정보를 강조한다는 점에서 보완적인 해석을 제공하였다. 이러한 분석 틀은 다른 라이브서비스 게임이나 공개 플랫폼 트래픽 데이터에도 적용 가능하며, 운영 이벤트가 시간대별 이용 구조를 어떻게 바꾸는지 이해하는 데 유용한 도구가 될 수 있다.
...
\newpage
\appendix
\section{PoE 리그 출시일 목록}
\begin{table}[H]
\centering
\caption{PoE 리그 출시일 목록}
\begin{tabular}{lll}
\toprule
순번 & 리그명 & 출시일 \\
\midrule
...
평활화 $\lambda$ & $10^{-2}$ \\
FPCA 개수 & 4개 주성분 \\
미분 함수 분석 & 평활화된 $\log(1+\mathrm{players})$ 곡선의 1차 미분 함수 사용 \\
